...
def find_cutomer(r,c):
    ...
    while q:
        ...
        if visited[x][y] > MIN:
            break
        if (x,y) in customer_start:
            MIN = visited[x][y] - 1
            candidate.append((x,y,MIN))
        for dir in dirs:
            nx = x + dir[0]
            ny = y + dir[1]
            if 0 <= nx < n and 0 <= ny < n:
                if arr[nx][ny] == 0 and visited[nx][ny] == 0:
                    ...
                    q.append((nx,ny))
    if candidate:
        candidate.sort(key=lambda data:(data[2],data[0],data[1]))
        return candidate[0][0],candidate[0][1],candidate[0][2]
    else:
        return -1,-1,-1

def find_end(r,c, end):
    q = deque()
    visited = [[0] * n for _ in range(n)]
    visited[r][c] = 1
    q.append((r,c))
...
for _ in range(m):
    ...
    customer_end.append((c - 1, d - 1))
for _ in range(m):
    x,y,dist = find_cutomer(st_r,st_c)
    if dist == -1 or fuel - dist < 0:
        print(-1)
        sys.exit()
    else:
        fuel -= dist
        # 같은 출발점, 같은 도착점이 있을 수 있으니 인덱스 주의해야함
        i = customer_start.index((x,y))
        customer_start[i] =[-1,-1]
        a,b,dist2 = find_end(x,y,i)
        if (a,b) != customer_end[i] or fuel - dist2 < 0:
            print(-1)
            sys.exit()
        fuel += dist2
        st_r,st_c = a,b
